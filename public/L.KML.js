...
L.Util.extend(L.KML, {
            ...
            for (var i = 0; i < xml.childNodes.length; i++) {
                ...
                if (key === "hotSpot") {
                    for (var j = 0; j < e.attributes.length; j++) {
                        options[e.attributes[j].name] =
                            e.attributes[j].nodeValue;
                    }
                } else {
                    var value = e.childNodes[0].nodeValue;
                    if (key === "color") {
                        options.opacity =
                            parseInt(value.substring(0, 2), 16) / 255.0;
                        options.color =
                            "#" +
                            value.substring(6, 8) +
                            value.substring(4, 6) +
    ...
    parsePlacemark: function (place, xml, style, options) {
        ...
        for (j in parse) {
            ...
            for (i = 0; i < el.length; i++) {
                var l = this["parse" + tag.replace(/gx:/, "")](
                    el[i],
                    xml,
                    opts
                );
    ...
    parseGroundOverlay: function (xml) {
        ...
        var bounds = new L.LatLngBounds(
            [
                latlonbox.getElementsByTagName("south")[0].childNodes[0]
                    .nodeValue,
                latlonbox.getElementsByTagName("west")[0].childNodes[0]
                    .nodeValue,
            ],
            [
                latlonbox.getElementsByTagName("north")[0].childNodes[0]
                    .nodeValue,
                latlonbox.getElementsByTagName("east")[0].childNodes[0]
                    .nodeValue,
            ]
        );
        ...
        if (latlonbox.getElementsByTagName("rotation")[0] !== undefined) {
            var rotation =
                latlonbox.getElementsByTagName("rotation")[0].childNodes[0]
                    .nodeValue;
...
L.KMLIcon = L.Icon.extend({
    ...
    applyCustomStyles: function (img) {
        ...
        if (options.anchorType.y === "fraction")
            img.style.marginTop =
                -(1 - options.anchorRef.y) * img.height + 1 + "px";
